...
// 當 startDate 改變時，自動修正 endDate 不超過 2 個月
...
const nextPage = () => {
  if (pagination.value.CurrentPage < pagination.value.TotalPages) {
    fetchData(pagination.value.CurrentPage + 1)
  }
}
...
const getStatusInfo = (status?: number) => {
  switch (status) {
    case BacktestStatus.Success:
      return { text: '達標', color: 'bg-green-100 text-green-800' }
    case BacktestStatus.Failed:
      return { text: '未觸發停損停利', color: 'bg-orange-100 text-orange-800' }
    case BacktestStatus.StopLoss:
      return { text: '停損', color: 'bg-red-100 text-red-800' }
    case BacktestStatus.Pending:
    default:
      return { text: '待回測', color: 'bg-gray-100 text-gray-600' }
  }
}
  ...
  <div class="min-h-screen bg-gray-100 flex flex-col pt-0">
    ...
    <main class="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      <div class="mb-8 flex justify-between items-center">
        <div>
          <h1 class="text-2xl font-bold text-gray-900">回測統計分析</h1>
          <p class="mt-1 text-sm text-gray-500">查詢指定區間內推薦的回測結果統計</p>
        </div>
      ...
      <div class="bg-white rounded-lg shadow p-4 mb-4">
        <div class="flex flex-wrap items-center gap-3">
          <span class="text-sm font-medium text-gray-700">查詢區間</span>
          <div class="inline-flex items-center border border-gray-300 rounded-lg overflow-hidden shadow-sm">
            <input
              id="startDate"
              v-model="startDate"
              type="date"
              ...
              class="px-3 py-2 border-0 focus:ring-0 sm:text-sm text-gray-700 bg-white"
            />
            <span class="px-2 text-gray-400 bg-gray-50 text-sm select-none">~</span>
            <input
              id="endDate"
              v-model="endDate"
              type="date"
      ...
      <!-- 警語 -->
      <div class="mb-6 p-3 bg-amber-50 border border-amber-200 rounded-lg flex items-start gap-2">
      ...
      <template v-else>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
          ...
          <div class="bg-white rounded-lg shadow p-5 border-l-4 border-green-500">
            <p class="text-sm font-medium text-gray-500">成功達標</p>
            <p class="mt-2 text-3xl font-bold text-green-600">{{ summary.Success }}</p>
          ...
          <div class="bg-white rounded-lg shadow p-5 border-l-4 border-orange-500">
            <p class="text-sm font-medium text-gray-500">未觸發停損停利</p>
            <p class="mt-2 text-3xl font-bold text-orange-600">{{ summary.Failed }}</p>
          ...
          <div class="bg-white rounded-lg shadow p-5 border-l-4 border-red-500">
            <p class="text-sm font-medium text-gray-500">觸發停損</p>
            <p class="mt-2 text-3xl font-bold text-red-600">{{ summary.StopLoss }}</p>
          ...
          <div class="bg-white rounded-lg shadow p-5 border-l-4 border-gray-400">
            <p class="text-sm font-medium text-gray-500">待回測</p>
            <p class="mt-2 text-3xl font-bold text-gray-600">{{ summary.Pending }}</p>
          ...
          <!-- Pie Chart -->
        <!-- Pie Chart -->
        <div class="bg-white rounded-lg shadow p-5 flex flex-col items-center justify-center mb-6">
          <p class="text-sm font-medium text-gray-500 mb-3">整體成功率</p>
          <div class="relative w-40 h-40">
            ...
            <div class="absolute inset-0 flex items-center justify-center">
              <div class="w-24 h-24 bg-white rounded-full flex items-center justify-center shadow-inner">
                <span class="text-2xl font-bold text-gray-800">{{ summary.SuccessRate }}%</span>
              </div>
            </div>
          </div>
          <!-- Legend -->
          <div class="mt-4 grid grid-cols-2 gap-x-6 gap-y-1 text-xs">
            <div v-for="slice in pieSlices" :key="slice.label" class="flex items-center gap-1.5">
              ...
              <span class="text-gray-600">{{ slice.label }} ({{ slice.count }})</span>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-4 mb-6 flex items-center justify-between">
          <span class="text-sm text-gray-600">
            總計 <strong class="text-gray-900">{{ summary.Total }}</strong> 筆推薦，
            成功率 <strong class="text-green-600">{{ summary.SuccessRate }}%</strong>
          </span>
        </div>
        ...
        <div class="bg-white rounded-lg shadow overflow-hidden">
            ...
            <table class="min-w-full divide-y divide-gray-200">
              <thead class="bg-gray-50">
                <tr>
                  ...
                  <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">股票代號</th>
                  <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">策略</th>
                  <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">推薦價</th>
                  <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">目標價</th>
                  <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">停損價</th>
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr v-else v-for="(record, index) in details" :key="index" class="hover:bg-gray-50 transition-colors">
          ...
          <!-- Pagination -->
          <div v-if="pagination.TotalPages > 1" class="bg-white px-4 py-3 border-t border-gray-200 flex items-center justify-between sm:px-6">
